...
default_cfgs = {
    ...
    'convformer_dcls_gauss_s18': _cfg(
        url='')
}
...
@register_model
def convformer_audio_s18(pretrained=False, **kwargs):
    model = MetaFormer(
        depths=[3, 3, 9, 3],
        dims=[64, 128, 320, 512],
    ...
    stem_audio = nn.Conv2d(1, 64, kernel_size=(2, 16), stride=(2, 16), bias=False)
    trunc_normal_(stem_audio.weight, std=0.02)
    model.downsample_layers[0].conv = stem_audio
    return model


@register_model
def convformer_dcls_audio_s18(pretrained=False, **kwargs):
    model = MetaFormer(
        depths=[3, 3, 9, 3],
        dims=[64, 128, 320, 512],
        token_mixers=SepConv,
    ...
    model.default_cfg = default_cfgs["convformer_dcls_gauss_s18"]
